...
| 類型 | 頻率 | 定位 | 產出 |
|:--|:--|:--|:--|
| **日報** | 不定期 | 新聞翻譯＋摘要（3-5 則精選） | `_posts/YYYY-MM-DD/YYYY-MM-DD-llm-daily.md` |
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  09:00/17:00（每天 2 次）   11:45（每 3 天）   12:05（每 3 天）   週日 13:00  │
│  ┌──────────────┐      ┌────────────────┐  ┌────────────┐  ┌────────┐│
│  │ ① 蒐集器     │ ──→ │ ② 日報發布器   │  │ ③ 中午匯總 │  │ ④ 週報 ││
...
| ID | 名稱 | 排程 | 模式 | 用途 |
|:--|:--|:--|:--|:--|
| `90a0d20fbc91` | LLM 新聞蒐集器 | `0 9,17 * * *` | no_agent + script | 跑 `llm-news-crawler` 蒐集新聞 |
| `6c8f226a0892` | LLM 新聞發布器 | `45 11 */3 * *` | LLM agent | 讀① output 寫日報到部落格 |
| `e0349bb0f3b1` | 中午新聞匯總 | `5 12 */3 * *` | LLM agent | 整理清單發 Telegram 給使用者挑文 |
...
```
匯總（每 3 天 12:05 Telegram）→ 使用者回編號（如 1,3,5,8）
                │
                ▼
